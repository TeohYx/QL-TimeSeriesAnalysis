...
def predict_LSTM_10days(db, today):
    ...
    if difference > 0:
        ...
        slope = f":green[~ {slope.round(2)}]"
    else:
        result = ":red[DECREASING]"
        slope = f":red[~ {slope.round(2)}]"
    # print(actual_prediction)
    st.markdown(f"Prediction of next {trained_model.n_forecast} days based on :orange[{reference_start}] to :orange[{reference_end}] (:orange[{trained_model.n_window}] days)")
    st.line_chart(combine)  

    st.markdown(f"The slopness is {slope}")
